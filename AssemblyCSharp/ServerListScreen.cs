...
public class ServerListScreen : mScreen, IActionListener
{
    ...
    public static string javaVN = "Vũ trụ 1:42.96.16.71:15880:0,0,0";

	public static string smartPhoneIn = "Vũ trụ 1:42.96.16.71:15880:0,0,0";

	public static string javaIn = "Vũ trụ 1:42.96.16.71:15880:0,0,0";

	public static string smartPhoneE = "Vũ trụ 1:42.96.16.71:15880:0,0,0";

	public static string javaE = "Vũ trụ 1:42.96.16.71:15880:0,0,0";
    ...
    public override void update()
	{
        ...
        if (delayAuto > 0)
        {
            ...
            if (delayAuto == 0)
            {
                Command cmd = new Command(string.Empty, this, 3, null);
				cmd.performAction();
            }
        }
        if (waitToLogin)
		...
		if (!loadScreen && (bigOk || percent == 100))
			cmdDownload = null;
		base.update();
        if (LoginScr.isLoginAnotherDevice)
		{
			Command cmd = new Command(mResources.OK, GameCanvas.gI(), 8882, null);
			cmd.performAction();
			LoginScr.isLoginAnotherDevice = false;
			GameCanvas.endDlg();
			this.switchToMe();
			delayAuto = -1;
        }

        if (Char.isLoadingMap || !loadScreen || !isAutoConect || GameCanvas.currentScreen != this || testConnect == 2)return;
